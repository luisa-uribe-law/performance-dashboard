...
export default function KpiCard({ label, subtitle, value, suffix = "", prefix = "", prevValue, color = "var(--accent)", invertDelta, deltaLabel }: KpiCardProps) {
  ...
  const wentUp = delta !== null && delta > 0;
  ...
  return (
    <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] px-3.5 py-3.5 hover:border-[var(--border-light)] hover:bg-[var(--card-hover)] transition-all duration-200 min-w-0 flex flex-col h-full">
      {/* Label — fixed height */}
      ...
      <div className="h-[18px] mt-1.5 flex items-center">
        {deltaNum !== null ? (
          <div className={`flex items-center gap-0.5 text-[10px] font-semibold leading-none ${isGood ? "text-[var(--success)]" : "text-[var(--danger)]"}`}>
