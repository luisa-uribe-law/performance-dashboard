...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Filter by actual deployedDate to match requested period (the DURING clause can
    // return tickets whose statuscategorychangedate falls outside the period)
    ...
    for (const t of demTasks) {
      ...
      const deployedMonth = deployedDate.slice(0, 7); // "2026-02"
